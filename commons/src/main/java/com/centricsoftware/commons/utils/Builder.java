...
/**
 * 通用的 Builder 模式构建器
 * 通过Supplier懒加载的特性保存需要创建的对象
 * 将每一步的操作保存到Consumer中，存入到List集合
 * 在build方法中通过Supplier的get()方法获取实例，在遍历list调用每个Consumer的accept方法给该实例的参数进行初始化
 * @author zheng.gong
 * @date 2020/8/27
 */
public class Builder<T> {
    ...
    final List<Consumer<T>> modifiers = new ArrayList<>();
